...
if($type == 2) {
    ...
    if($time == 1) {
        ...
        $Quotes = new getKFuture($cacheFile, 1);
    }
} else {
    if($time && !in_array($time,[1,5,15,30,60])) {
        $result['status'] = "04";
    ...
    $Quotes = new getInternationKMin($cacheFile, 0,$time1);
}
    ...
    if($data) {
        foreach ($data['result'] as $key=> $list) {
            foreach ($list as $k=> $v) {
                $arr = explode(',',$v);
                if(!$k) $minPrice = $arr[1];
                if($maxPrice < $arr[1]) $maxPrice = $arr[1];
                if($minPrice > $arr[1]) $minPrice = $arr[1];
                if($k%$time==0 && $k) {
                    $res[] = date('Y-m-d H:i',strtotime($key.' '.$arr[0])-$time*60).','.$minPrice.','.$maxPrice.','.$minPrice.','.$maxPrice.','.$num;
                    $maxPrice = $arr[1];
                    $minPrice = $arr[1];
                    $num = 0;
                }
